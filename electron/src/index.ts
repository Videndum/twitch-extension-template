...
class Desktop extends GameHandler {
  ...
  overlayHandler?: OverlayHandler | null = new OverlayHandler()

  constructor() {
    super()
    if (!this.checkPlatform()) throw new Error(`The platform you are currently using is not supported. Please change platform to access this feature. You are currently on ${this.CurOS}`)
  ...
  async createOverlay() {
    ...
    if (game.WindowTitle) overlayWindow.attachTo(this.overlay, game.WindowTitle)
    this.overlayContent()
